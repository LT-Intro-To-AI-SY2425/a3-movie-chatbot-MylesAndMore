...
def title_by_year_range(matches: List[str]) -> List[str]:
    ...
    return [get_title(movie) for movie in movie_db if start <= get_year(movie) <= end]
...
def director_by_title(matches: List[str]) -> List[str]:
    ...
    """
    title = matches[0]
    return [get_director(movie) for movie in movie_db if get_title(movie) == title]
...
def actors_by_title(matches: List[str]) -> List[str]:
    ...
    """
    title = matches[0]
    return [actor for movie in movie_db for actor in get_actors(movie) if get_title(movie) == title]
